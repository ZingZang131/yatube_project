...
│
├── ANTOBAKA/                 ← Главный проект Django
│   ├── __init__.py           → делает папку Python-пакетом
│   ├── asgi.py               → запуск через ASGI (WebSocket, async)
│   ├── settings.py           → настройки проекта
│   ├── urls.py               → главные маршруты сайта
│   └── wsgi.py               → запуск через WSGI
│
...
│   ├── models.py             → модели БД
│   ├── serializers.py        → сериализация данных (API/DRF)
│   ├── tests.py              → тесты
...
│   ├── about/                → страницы "О нас"
│   ├── includes/             → общие части шаблонов
